...
/// Parse a quiz file content and return (optional_title, questions)
pub fn parse_quiz_file(content: &str) -> anyhow::Result<(Option<String>, Vec<Question>)> {
    ...
    for (i, block) in blocks.iter().enumerate() {
        ...
        // First block may contain a title
        if i == 0 {
            ...
            if let Some(line) = first_non_comment_non_empty {
                ...
                let non_comment_lines: Vec<_> = block
                    .lines()
                    .filter(|l| !l.trim().is_empty() && !l.trim().starts_with('#'))
                    .collect();
                if non_comment_lines.len() == 1
                    && (line.starts_with("标题：") || line.starts_with("Title:"))
                {
                    continue;
                }
            }
        ...
        for line in block.lines() {
            ...
            if let Some(v) = line.strip_prefix("题型：") {
                ...
                match v {
                    ...
                    "多选题" | "Multiple" | "multiple" | "MULTIPLE" => {
                        qtype = "多选".to_string()
                    }
                    "填空题" | "Fill" | "fill" | "FILL" | "Blank" | "blank" => {
                        qtype = "填空".to_string()
                    }
                    "判断题" | "Judgment" | "judgment" | "True/False" | "true/false" | "TF" => {
                        qtype = "判断".to_string()
                    }
                    _ => qtype = v.to_string(),
                }
        ...
        anyhow::ensure!(
            !qtype.is_empty() && !question_text.is_empty(),
            "缺少题型或题目"
        );
        ...
        let question = match qtype.as_str() {
            "单选" => {
                ...
                anyhow::ensure!(correct >= 1, "单选题答案序号不能小于1");
            ...
            "多选" => {
                ...
                for idx in &raw_list {
                    anyhow::ensure!(*idx >= 1, "多选题答案序号不能小于1");
                }
            ...
            "填空" => {
                let delimiter = if answer_str.contains('；') {
                    '；'
                } else {
                    ';'
                };
