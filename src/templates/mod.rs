...
pub trait Template: Send + Sync {
    fn name(&self) -> &str;
    fn generate_html(&self, title: &str, questions_json: &str, generate_time: &str, locale: &Locale) -> String;
...
impl Template for QuizTemplate {
    ...
    fn generate_html(&self, title: &str, questions_json: &str, generate_time: &str, locale: &Locale) -> String {
        let quiz_count_prefix = locale.tr("html_quiz_count_prefix");
        let quiz_count_suffix = locale.tr("html_quiz_count_suffix");
        let prev_btn = locale.tr("html_prev_btn");
        let next_btn = locale.tr("html_next_btn");
        let submit_btn = locale.tr("html_submit_btn");
        let reset_btn = locale.tr("html_reset_btn");
        let score_text = locale.tr("html_score_text");
        let submitted = locale.tr("html_submitted");
        let all_done = locale.tr("html_all_done");
        let fill_placeholder = locale.tr("html_fill_placeholder");
        let correct_answer_prefix = locale.tr("html_correct_answer_prefix");
        let or = locale.tr("html_or");
        let separator = locale.tr("html_separator");
        let true_text = locale.tr("html_true");
        let false_text = locale.tr("html_false");
        let type_single = locale.tr("html_type_single");
        let type_fill = locale.tr("html_type_fill");
        let type_judge = locale.tr("html_type_judge");
        let type_multiple = locale.tr("html_type_multiple");
        let correct_icon = locale.tr("html_correct_icon");
        let wrong_icon = locale.tr("html_wrong_icon");
        let perfect = locale.tr("html_perfect");
        let excellent = locale.tr("html_excellent");
        let good = locale.tr("html_good");
        let keep_going = locale.tr("html_keep_going");
        let try_harder = locale.tr("html_try_harder");
        let fill_label_prefix = locale.tr("html_fill_label_prefix");
...
<div class="main-container" id="mainContainer">
    <div class="header">
        ...
        <div class="subtitle" id="subtitle">{quiz_count_prefix}<span id="totalQuestions">0</span>{quiz_count_suffix}</div>
    ...
    <div class="btn-row">
        <button class="btn btn-prev" id="btnPrev"> {prev_btn}</button>
        <button class="btn btn-submit" id="btnSubmit"> {submit_btn}</button>
        <button class="btn btn-next" id="btnNext">{next_btn} </button>
    </div>
    ...
    <div class="score-bar" id="scoreBar">
        ...
        <button class="btn btn-reset" id="btnReset"> {reset_btn}</button>
    </div>
</div>
...
<script>
    (function() {{
        ...
        function showScoreIfAllSubmitted() {{
            ...
            if (correct === total) dom.scoreEmoji.textContent = '{perfect}';
            else if (correct >= total * 0.8) dom.scoreEmoji.textContent = '{excellent}';
            else if (correct >= total * 0.6) dom.scoreEmoji.textContent = '{good}';
            else if (correct >= total * 0.4) dom.scoreEmoji.textContent = '{keep_going}';
            else dom.scoreEmoji.textContent = '{try_harder}';
            dom.scoreBar.classList.add('show');
            dom.btnReset.style.display = 'inline-block';
            dom.btnSubmit.disabled = true;
            dom.btnSubmit.textContent = '{all_done}';
        ...
        function render() {{
            ...
            const typeLabels = {{ single: '{type_single}', fill: '{type_fill}', judge: '{type_judge}', multiple: '{type_multiple}' }};
            ...
            if (q.type === 'single') {{
                ...
                for (let bi = 0; bi < blankCount; bi++) {{
                    ...
                    const label = blankCount > 1 ? `{fill_label_prefix}${{bi+1}}：` : '';
                    html += `<div class="fill-single">${{label}}<input type="text" class="${{inputCls}}" id="fillInput${{index}}_${{bi}}" value="${{escapeHtml(val)}}" 
                               placeholder="{fill_placeholder}" ${{submitted ? 'disabled' : ''}} autocomplete="off"></div>`;
                }}
                html += `</div>`;
                if (submitted && !isAnswerCorrect(index)) {{
                    const correctText = q.answers.map(group => group.join('{or}')).join('{separator}');
                    html += `<div class="fill-correct-answers">{correct_answer_prefix}${{correctText}}</div>`;
                }}
            }} else if (q.type === 'judge') {{
                html += `<div class="judge-options">`;
                const opts = [{{v:true, l:'{true_text}'}}, {{v:false, l:'{false_text}'}}];
                opts.forEach(o => {{
                    let cls = 'judge-btn';
            ...
            if (submitted) {{
                const icon = isAnswerCorrect(index) ? '{correct_icon}' : '{wrong_icon}';
        ...
        function updateProgressAndButtons() {{
            ...
            if (allSubmitted()) {{
                ...
                dom.btnSubmit.disabled = true;
                dom.btnSubmit.textContent = '{submitted}';
            }} else {{
                dom.btnSubmit.disabled = false;
                dom.btnSubmit.textContent = '{submit_btn}';
            }}
        }}
        ...
        render();
            ...
            quiz_count_prefix = quiz_count_prefix,
            quiz_count_suffix = quiz_count_suffix,
            prev_btn = prev_btn,
            next_btn = next_btn,
            ...
            fill_label_prefix = fill_label_prefix,
